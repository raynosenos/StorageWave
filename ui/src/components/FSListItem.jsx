...
const FSListItem = (props) => {
	...
	const fileIcon = () => {
		if (!props.fsElement.is_file) return null
		return getFileIcon(props.fsElement.name)
	}
		...
		<>
			<ListItem disablePadding>
				<ListItemButton onClick={handleNavigate}>
					<ListItemIcon>
						<Show when={props.fsElement.is_file} fallback={<FolderIcon color="primary" />}>
							<Box sx={{
								fontSize: '1.5rem',
								width: 24,
								height: 24,
					...
					<ListItemText
						...
						secondary={props.fsElement.is_file ? getFileExtension(props.fsElement.name).toUpperCase() : 'Folder'}
					/>
				</ListItemButton>
				<IconButton
					onClick={(event) => {
